# Makefile for building dynamic libraries, examples, and tests for GSDK

# Use clang on macOS
CC = clang
CFLAGS = -fPIC -Wall -Wextra -O2 -Iinclude
# macOS specific flags for dynamic libraries
DYLIB_FLAGS = -dynamiclib -install_name @rpath/$(@F)
# Set rpath to find libraries in the lib directory relative to the executable
RPATH_FLAGS = -Wl,-rpath,@executable_path/../lib
# Link against pthread for POSIX thread support
LDFLAGS = -lpthread
...
# Core libraries first, as other libraries depend on them
CORE_LIBS = log sync hash_cache socket
DATA_LIBS = array circular_buffer dict tree tuple priority_queue queue stack
REFLECTION_LIBS = json base64
PERFORMANCE_LIBS = parallel

# Combined list of all libraries
...
$(BUILD_EXAMPLE_DIR)/hash_cache_example: $(EXAMPLES_DIR)/hash_cache_example.c | $(BUILD_EXAMPLE_DIR)
	$(CC) $(CFLAGS) $(RPATH_FLAGS) -o $@ $^ $(ROOT_DIR)/$(BUILD_LIB_DIR)/log.dylib $(ROOT_DIR)/$(BUILD_LIB_DIR)/hash_cache.dylib
