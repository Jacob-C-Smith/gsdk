...
int main ( int argc, const char *argv[] )
{
    ...
    p_signature = default_allocator(0, sizeof(long long) * 2);
    if ( p_signature == NULL ) goto failed_to_allocate_signature;

    // read the signature
    if ( 0 == fread(p_signature, sizeof(char), sizeof(long long) * 2, signature_file) ) goto failed_to_read_signature;

    // verify the signature
    int verification_result = dsa_verify(
        p_public_key,
        NULL,
        _buffer, 
        message_len,
